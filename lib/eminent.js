...
var _common = require('emmet/lib/utils/common');
...
var compareNode = function compareNode(node, tree, hasAttrs, isAttrs, isContent) {
    ...
    if (isContent && tree.content !== '' && tree._name !== '') {
        var value = _common2.default.replaceCounter(tree.content, tree.counter, tree.maxCount);
    ...
    if (nodeChildren.length !== treeChildren.length) {
        _assert2.default.fail(_getNodeHTML(node, true), _getTreeHTML(tree, true), 'Different number of DOM child nodes', '!=');
    }
